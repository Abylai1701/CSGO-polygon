...
class NetworkRouter: NetworkManager {
    ...
    func request<T: Decodable>(_ route: EndpointType, completion: @escaping ((Result<T>, Int)) -> Void) {
        ...
        do {
            ...
            completion((.failure(error.localizedDescription), 400))
        }
        self.task?.resume()
    ...
    func arrayToData(_ array: [Int]) -> Data? {
